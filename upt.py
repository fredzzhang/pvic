...
class FeatureHead(nn.Module):
    def __init__(self, dim, num_enc_layers, feature_level):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, features, image_padding_mask, pos_embeds, pe):
        ...
        ms_mem = memory.split(spatial_shapes.prod(1).tolist(), dim=1)
        ms_mask = mask_flatten.split(spatial_shapes.prod(1).tolist(), dim=1)

        return ms_mem[self.feature_level], pos_embeds[self.feature_level], ms_mask[self.feature_level]
...
class UPT(nn.Module):
    ...
    def forward(self,
        ...
        memory, pos, mask = self.feature_head(
...
def build_detector(args, obj_to_verb):
    ...
    level = {"c3": 0, "c4": 1, "c5": 2, "c6": 3}
    feature_head = FeatureHead(
        args.hidden_dim,
        args.triplet_enc_layers,
        level[args.kv_feature_level]
    )
    detector = UPT(
